...
%       restart   Struct that can contain the largest singular value s1
...
%       end
    ...
    atma = @(x) s.*Afun( M.*Afun(x,'notransp') , 'transp' );
    optionsEIGS.disp = 0;
    ...
    if isfield(options,'lambda')
        ...
        if ~ischar(lambda)
            ...
            elseif strncmpi(lambda,'psi1',4)
                ...
                % Check if the largest singular value is known.
                if isfield(options,'restart') && ...
                        isfield(options.restart,'s1')
                    sigma1tildesquare = options.restart.s1^2;
                else
...
while ~stop
    ...
    if casel == 1
        ...
        xk = xk + lambda*(s.*Afun(M.*rxk,'transp'));  %(A'*(M.*rxk)));
    elseif casel == 2
    ...
    elseif casel == 3
        ...
        xk = xk + lambdak(ite)*(s.*Afun(M.*rxk,'transp'));  %(A'*(M.*rxk)));
